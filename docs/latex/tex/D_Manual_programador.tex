...
Para trabajar con el proyecto es necesario disponer de un entorno de desarrollo web basado en Node.js y npm \cite{nodejs,npm}. La aplicación se ejecuta completamente en el navegador y no requiere la instalación de un servidor de bases de datos ni de un backend propio.
...
\item \textbf{Editor de código}: se puede utilizar cualquier editor compatible con proyectos JavaScript. Durante el desarrollo se ha trabajado principalmente con Visual Studio Code \cite{vscode}.
...
La carpeta \texttt{build/} contiene los ficheros estáticos resultantes, preparados para ser servidos por una plataforma de despliegue web. En el proyecto actual, el despliegue se realiza sobre Vercel \cite{vercel}, que se encarga de construir y publicar la aplicación a partir del repositorio.
...
La aplicación se organiza en torno a una separación progresiva entre la interfaz React \cite{react}, la integración con mxGraph \cite{mxgraph}, las reglas propias del modelo Entidad--Relación, la transformación al modelo relacional y la generación de SQL. Esta separación no elimina por completo el acoplamiento propio de una aplicación heredada, pero permite localizar mejor las responsabilidades principales del sistema.
...
Esta carpeta contiene funciones relacionadas con entidades, relaciones, atributos, composición del diagrama, normalización, ejemplos predefinidos, jerarquías ISA y validación. Estas reglas se apoyan en los conceptos del modelo E/R y su transformación relacional descritos en el material teórico de referencia \cite{tema6}.
...
\subsection{Métricas del código fuente}
...
La medición se realizó sobre las carpetas \texttt{src/}, \texttt{tests/} y \texttt{scripts/}, excluyendo dependencias instaladas, artefactos de construcción, recursos estáticos generados y documentación externa.

\begin{table}[H]
\centering
\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{p{0.52\textwidth} r r}
\toprule
\textbf{Bloque analizado} & \textbf{Ficheros} & \textbf{Líneas} \tabularnewline
\midrule
Código de producción en \texttt{src/} & 58 & 16.184 \tabularnewline
Pruebas y utilidades de prueba en \texttt{tests/} & 53 & 14.377 \tabularnewline
Scripts auxiliares en \texttt{scripts/} & 1 & 38 \tabularnewline
\midrule
\textbf{Total considerado} & \textbf{112} & \textbf{30.599} \tabularnewline
\bottomrule
\end{tabular}
\caption{Métricas básicas de tamaño del código fuente y pruebas del proyecto.}
\label{tab:metricas_codigo_fuente}
\end{table}

Como indicador complementario de complejidad estructural, se revisó la distribución del código dentro de \texttt{src/}. La mayor parte del tamaño se concentra en la interfaz del editor y en la lógica de dominio, lo cual es coherente con la naturaleza del proyecto: una aplicación web interactiva basada en React y mxGraph.
...
\begin{tabular}{p{0.52\textwidth} r r}
\toprule
\textbf{Carpeta de \texttt{src/}} & \textbf{Ficheros} & \textbf{Líneas} \tabularnewline
\midrule
\texttt{src/components/} & 27 & 10.194 \tabularnewline
\texttt{src/domain/} & 23 & 4.571 \tabularnewline
\texttt{src/i18n/} & 2 & 969 \tabularnewline
\texttt{src/services/} & 2 & 396 \tabularnewline
\bottomrule
\end{tabular}
\caption{Distribución aproximada del código de producción por carpetas principales.}
\label{tab:metricas_codigo_carpetas}
\end{table}
...
El análisis estático se realiza mediante Biome \cite{biome}. Esta herramienta permite detectar problemas de estilo, código no utilizado, errores simples y otras inconsistencias en el código fuente.
...
Las pruebas unitarias se ejecutan con Vitest \cite{vitest}. Estas pruebas se centran principalmente en la lógica interna de la aplicación, como validación del diagrama, transformación al modelo relacional, generación SQL, persistencia, normalización de datos y funciones auxiliares del dominio.
...
El proyecto también dispone de pruebas de extremo a extremo mediante Playwright \cite{playwright}. Estas pruebas permiten verificar flujos completos de uso desde el punto de vista del usuario, ejecutando la aplicación en un navegador controlado por la herramienta.
