...
Esta fase es especialmente importante en el contexto del proyecto porque actúa como puente entre el diagrama visual diseñado por el usuario y la salida SQL generada por la aplicación.
...
En la herramienta, esta generación se realiza intentando mantener un resultado legible y coherente con el modelo relacional obtenido. Por ejemplo, cuando una clave foránea referencia la clave primaria completa de otra tabla, la salida puede expresarse de forma compacta sin repetir información que ya queda definida en la tabla referenciada.
...
El modelo Entidad-Relación incluye más posibilidades de las que se han incorporado en la herramienta. Por ello, este capítulo se ha centrado en los conceptos que resultan necesarios para entender la evolución realizada: entidades, atributos, relaciones binarias y ternarias, entidades débiles, atributos compuestos y multivaluados, transformación al modelo relacional, generación de SQL y un soporte inicial para jerarquías ISA.
...
Las agregaciones se han tenido en cuenta como concepto teórico, pero no forman parte del alcance implementado. Esta delimitación permite ajustar el desarrollo al objetivo principal del trabajo: evolucionar el editor heredado manteniendo la coherencia entre el diagrama visual, el modelo interno, la validación, la transformación relacional y la generación de SQL.