...
El desarrollo de este trabajo no comenzó desde una aplicación vacía, sino a partir de un editor de diagramas Entidad-Relación que ya existía y que tenía una parte funcional implementada. Antes de añadir nuevas funcionalidades, fue necesario entender cómo estaba construido el sistema, qué partes podían aprovecharse y qué aspectos convenía revisar para poder seguir ampliándolo con mayor seguridad.
...
El desarrollo se organizó de forma incremental, utilizando ramas, commits, issues y milestones de GitHub para separar las tareas y mantener trazabilidad entre los problemas detectados, las soluciones aplicadas y las comprobaciones posteriores.

Esta organización resultó especialmente útil al trabajar sobre una aplicación heredada, donde un cambio podía afectar simultáneamente a la interfaz, el modelo interno, la persistencia, la validación o la generación de SQL. Por ello, se intentó que cada bloque de trabajo pudiera revisarse antes de continuar con nuevas ampliaciones.

Además, algunas decisiones de alcance se ajustaron durante el desarrollo a partir de la revisión del tutor. Esto fue especialmente relevante en las fases finales, donde se decidió mantener un soporte inicial y controlado para las jerarquías ISA y aplazar funcionalidades más complejas, como las agregaciones o la herencia múltiple real.
...
La Figura \ref{fig:modelo_interno_editor} resume la relación entre el lienzo visual del editor, el modelo interno y las operaciones que dependen de esa representación.
...
Por tanto, las ampliaciones del editor debían reflejarse también en esta estructura interna.
...
La Figura \ref{fig:dialogo_validacion} muestra un ejemplo de los errores detectados durante la validación, presentados de forma agrupada para facilitar su revisión por parte del usuario.

\imagen{dialogo_validacion}{Diálogo de validación con errores agrupados del diagrama.}{0.82}
...
Durante la revisión del generador se buscó producir un script más claro y legible. Para ello, las claves foráneas se incluyen dentro de las sentencias \texttt{CREATE TABLE} cuando las dependencias entre tablas pueden resolverse mediante el orden de creación. Las sentencias \texttt{ALTER TABLE} se mantienen como mecanismo de respaldo para los casos en los que aparecen dependencias cíclicas reales.

También se simplificó la representación de algunas claves foráneas cuando estas referencian la clave primaria completa de la tabla destino, evitando repetir información que ya queda definida en dicha tabla. Esta decisión no modifica la estructura relacional obtenida, sino la forma en la que se expresa el script final.

Asimismo, se realiza una normalización de los identificadores utilizados en el SQL. Esta normalización ayuda a evitar problemas con nombres que puedan contener espacios, acentos u otros caracteres poco adecuados para identificadores de base de datos. Además, cuando se detectan posibles repeticiones de nombres dentro de una misma tabla, se ajustan para evitar conflictos en el esquema resultante.
...
La Figura \ref{fig:acciones_contextuales_editor} muestra un ejemplo de las acciones contextuales disponibles al seleccionar un elemento del diagrama, lo que permite concentrar las opciones relevantes según el tipo de elemento activo.

\imagen{acciones_contextuales_editor}{Acciones contextuales mostradas por el editor al seleccionar un elemento del diagrama.}{0.82}

También se añadieron pequeños textos de ayuda en zonas concretas del editor y se refinó el comportamiento de la selección múltiple. El rectángulo de selección se hizo visible y, cuando existen varios elementos seleccionados, la barra lateral evita mostrar acciones propias de un único elemento. En ese caso se mantienen únicamente acciones aplicables al conjunto seleccionado, como el desplazamiento en el lienzo o el borrado conjunto.

\subsection{Mejora de validaciones, diálogos y mensajes de ayuda}

Otro bloque de trabajo fue la revisión de los mensajes de validación y de los diálogos de confirmación. Antes de generar SQL y en determinadas operaciones del editor, la aplicación valida la estructura del modelo. En esta fase se mejoró la forma de presentar los errores detectados, agrupándolos de manera más clara y mostrando, cuando es posible, el elemento concreto relacionado con el problema.
...
Además, se revisaron los textos de confirmación de acciones sensibles, como generar SQL, importar un diagrama JSON o reiniciar el lienzo. También se añadieron textos breves de ayuda en los diálogos relacionados con jerarquías ISA y con roles en relaciones ternarias. Los diálogos más complejos pueden moverse por la pantalla, evitando que oculten de forma permanente la parte del diagrama que el usuario necesita consultar.

\subsection{Operaciones globales sobre el diagrama}

También se revisaron varias acciones que afectan al diagrama completo. Además de la importación y exportación en JSON, se añadió la posibilidad de exportar la representación visual del diagrama como imagen en formato PNG o SVG. Esta exportación se limita al aspecto visual del modelo y no modifica el JSON, la persistencia interna ni el SQL generado.

Se incorporó además una acción explícita para comprobar el diagrama sin generar SQL. De esta forma, el usuario puede revisar el estado del modelo directamente: si el diagrama es válido recibe una notificación de éxito, y si contiene errores se muestra el diálogo de validación agrupado.

Otra mejora fue la generación de estructuras básicas de ejemplo, como relaciones binarias, relaciones ternarias, entidades débiles o jerarquías ISA sencillas. Esta funcionalidad sirve como apoyo para probar la herramienta y crear modelos base, pero no pretende actuar como un sistema avanzado de plantillas ni realizar fusiones semánticas entre elementos equivalentes.

La Figura \ref{fig:generacion_estructuras} muestra la generación de una estructura básica de ejemplo, utilizada como apoyo para crear rápidamente patrones habituales del modelo Entidad-Relación.

\imagen{generacion_estructuras}{Generación de una estructura básica de ejemplo en el editor.}{0.82}

Por último, se añadieron mejoras de apoyo a la edición, como el ajuste de vista al contenido del diagrama, la ayuda integrada, la ventana de información del proyecto y un soporte básico de idioma español/inglés para los textos principales de la interfaz. Estas mejoras no cambian la notación académica del modelo Entidad-Relación, pero ayudan a que la herramienta resulte más clara durante la edición.
...
Por este motivo, la importación combinada de JSON y la inserción de estructuras de ejemplo se plantearon de forma controlada. El usuario puede elegir entre reemplazar el diagrama actual o insertar nuevo contenido en el diagrama existente. En el segundo caso, la aplicación remapea identificadores internos, actualiza referencias, resuelve conflictos de nombres mediante renombrado y desplaza visualmente el bloque insertado.

Esta solución no intenta realizar una fusión semántica avanzada entre diagramas, pero permite combinar contenido de forma práctica y coherente con el alcance del proyecto.
